...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "TRFNetwork") type = TRFNetwork;
    if(strType == "TRFNetworkVariable") type = TRFNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == TRFNetwork) s = "TRFNetwork";
    if(type == TRFNetworkVariable) s = "TRFNetworkVariable";
