...
const char * const BITCOIN_CONF_FILENAME = "TRF.conf";
const char * const BITCOIN_PID_FILENAME = "TRFd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "TRF" is a composite category enabling all TRF-related debug output
            if(ptrCategory->count(string("TRF"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TRFCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TRFCore
    // Mac: ~/Library/Application Support/TRFCore
    // Unix: ~/.TRFcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TRFCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TRFCore";
#else
    // Unix
    return pathRet / ".TRFcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty TRF.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override TRF.conf
